    ...
    def createControls(self, color, shapeName):
        ...
        if len(self.sels) > 0:
            for sel in self.sels:
                ...
                self.renameCtrl = cmds.rename(sel + "_Ctrl")
                self.groupies = cmds.group(self.renameCtrl, world=True, name=(self.renameCtrl + "_Grp"))
                self.ColorControls(self.renameCtrl, color)
                cmds.matchTransform(self.groupies, sel)

        else:
            cmds.warning("Nothing selected Creating control and group at origin")
            self.ctrl = self.ControlTypes(shapeName)
            self.renameCtrl = cmds.rename("Basic_Ctrl")
            self.groupies = cmds.group(cmds.renameCtrl, world=True, name=(cmds.renameCtrl + "_Grp"))
            self.ColorControls(self.renameCtrl, color)

    def ControlTypes (self, shape):

        if shape == "Diamond":
            self.ctrls = cmds.curve(d=1, p=[(-0.5, 0, -0.5), (-0.25, 0, 0), (-0.5, 0, 0.5), (0, 0, 0.25), (0.5, 0, 0.5),
                                            (0.25, 0, 0), (0.5, 0, -0.5), (0, 0, -0.25)], k=[0, 1, 2, 3, 4, 5, 6, 7])
            self.ctrls = cmds.closeCurve(ch=True, ps=True, rpo=True, bb=0.5, bki=False, p=0.1)

        elif shape == "Square":
            self.ctrls = cmds.curve(d=1, p=[(-0.5, 0, -0.5), (-0.5, 0, 0.5), (0.5, 0, 0.5), (0.5, 0, -0.5)],
                                    k=[0, 1, 2, 3])
            self.ctrls = cmds.closeCurve(ch=True, ps=True, rpo=True, bb=0.5, bki=False, p=0.1)
